...
internal sealed partial class PcmAudioPlayer : IDisposable
{
    ...
    public void Play(String path, TimeSpan position, Double volume)
    {
        lock (_sync)
        {
            ...
            Int64 requested = (Int64)Math.Floor(
                Math.Max(0D, position.TotalSeconds) * AverageBytesPerSecond);
            Int32 offset = checked((Int32)Math.Min(
                audio.Length,
                requested - requested % BlockAlignment));
    ...
    {
        UInt32 level = checked((UInt32)Math.Round(
            Math.Clamp(volume, 0D, 1D) * UInt16.MaxValue));
    ...
    private static void EnsureSuccess(UInt32 result, String action)
    {
        if (result != 0)
        {
            throw new InvalidOperationException(
                $"Windows could not {action} ({result}).");
        }
    }
    ...
    [LibraryImport("winmm.dll")]
    private static partial UInt32 waveOutPrepareHeader(
        IntPtr handle,
        IntPtr header,
        UInt32 size);

    [LibraryImport("winmm.dll")]
    private static partial UInt32 waveOutUnprepareHeader(
        IntPtr handle,
        IntPtr header,
        UInt32 size);

    [LibraryImport("winmm.dll")]
    private static partial UInt32 waveOutWrite(
        IntPtr handle,
        IntPtr header,
        UInt32 size);
