...
public sealed class CliMainWindow : Runnable
{
    ...
    public CliMainWindow(CliBrowserServices services, String? initialAddress)
    {
        ...
        Title = services.IsPrivate ? "Lumi Private" : "Lumi";
        SchemeName = "Base";
        _tabs = new Tabs
        {
            X = 0,
    ...
    public void OpenMenu(CliTabView view, String category = "All")
    {
        ...
        IReadOnlyList<CliMenuEntry> entries = category switch
        {
            "File" => new[]
            {
                ...
                new CliMenuEntry("quit", "Quit Lumi", "Ctrl+Q")
            },
            "View" => new[]
            {
            ...
            "Help" => new[]
            {
                ...
                new CliMenuEntry("about", "About Lumi CLI")
            },
            _ => new[]
            {
                new CliMenuEntry("new", "New tab", "Ctrl+T"),
        ...
        CliMenuEntry? selected = CliDialogs.Choose(App, category == "All" ? "Lumi" : category, entries);
        if (selected is null)
        {
            return;
        }
    ...
    private async Task InitializeAsync()
    {
        ...
        {
            ShowError("Lumi", exception.Message);
        }
    ...
    private void ShowAbout()
    {
        if (App is not null)
        {
            CliDialogs.Show(
                App,
                "About Lumi CLI",
                "Lumi CLI\nThe semantic terminal viewer for LUMUI.\nRuntime " + Environment.Version);
        }
    }
    ...
    private void RequestQuit()
    {
        ...
        if (_tabManager.Tabs.Count > 1
            ...
            && !CliDialogs.Confirm(App, "Quit Lumi", "Close all open tabs?"))
        {
            return;
        }
